...
//Actions
const FETCH = 'KRO/STATUSES/FETCH';
...
//Async Thunks
export const fetchStatuses = token => dispatch => {
  dispatch({ type: FETCH });
  new Api(token)
    .get('statuses')
    .then(response => {
    ...
    .catch(error => {
      fetchStatusesHandleError(dispatch, error);
    });
};

//Helper Functions
const fetchStatusesHandleError = (dispatch, error) => {
  console.log(error);
