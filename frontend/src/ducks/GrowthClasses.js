...
//Actions
const FETCH = 'KRO/GROWTHCLASSES/FETCH';
...
export const fetchGrowthClasses = token => dispatch => {
  dispatch({ type: FETCH });
  new Api(token)
    .get('growth-classes')
    .then(response => {
    ...
    .catch(error => {
      fetchGrowthClassesHandleError(dispatch, error);
    });
};

//Helper Functions
const fetchGrowthClassesHandleError = (dispatch, error) => {
  console.log(error);
