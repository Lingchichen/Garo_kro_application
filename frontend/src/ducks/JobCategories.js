...
//Actions
const FETCH = 'KRO/JOBCATEGORIES/FETCH';
...
export const fetchJobCategories = token => dispatch => {
  dispatch({ type: FETCH });
  new Api(token)
    .get('job-categories')
    .then(response => {
    ...
    .catch(error => {
      fetchJobCategoriesHandleError(dispatch, error);
    });
};

//Helper Functions
const fetchJobCategoriesHandleError = (dispatch, error) => {
  console.log(error);
