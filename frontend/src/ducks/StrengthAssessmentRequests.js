...
//Actions
const FETCH = "KRO/STRENGTHASSESSMENTREQUESTS/FETCH";
...
export const fetchStrengthAssessmentRequests = (
  token,
  callback = null
) => dispatch => {
  dispatch({ type: FETCH });
  new Api(token)
    .get("strength-assessment-requests")
    .then(response => {
      dispatch({ type: FETCH, status: SUCCESS, payload: response.data });
      if (callback) callback();
    })
    .catch(error => {
      fetchStrengthAssessmentRequestsHandleError(dispatch, error);
    });
};

//Helper Functions
const fetchStrengthAssessmentRequestsHandleError = (dispatch, error) => {
  console.log(error);
