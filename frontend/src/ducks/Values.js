...
export const fetchValues = (token, callback = null) => dispatch => {
  dispatch({ type: FETCH });
  new Api(token)
    .get("values")
    .then(response => {
    ...
    .catch(error => {
      fetchValuesHandleError(dispatch, error);
    });
};

//Helper Functions
const fetchValuesHandleError = (dispatch, error) => {
  console.log(error);
