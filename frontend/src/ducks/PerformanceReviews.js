...
//Actions
const FETCH = "KRO/PERFORMANCEREVIEW/FETCH";
const SAVE = "KRO/PERFORMANCEREVIEW/SAVE";
...
//Async Thunks
export const fetchPerformanceReviews= (token, callback=null) => {
  return (dispatch) => {
    dispatch({ type: FETCH });
    new Api(token)
      ...
      .catch(error => {
        fetchPerformanceReviewsError(dispatch, error);
      });
  };
};
export const updateReview = (
  ...
  dispatch({ type: SAVE });
  new Api(token)
    .put("performance-reviews", review.id,review)
    .then(response => {
      dispatch({ type: SAVE, status: SUCCESS });
      dispatch(
        fetchPerformanceReviews(token, () => {
          if (callback) callback(response.data);
        })
      );
    })
    .catch(error => {
      saveReviewHandleError(dispatch, error);
...
export const createReview = (token, review, callback = null) => dispatch => {
  dispatch({ type: SAVE });
  new Api(token)
    .post("performance-reviews", review)
    .then(response => {
      dispatch({ type: SAVE, status: SUCCESS });
      dispatch(
        fetchPerformanceReviews(token, () => {
...
//Helper Functions
const fetchPerformanceReviewsError = (dispatch, error) => {
  console.log(error);
...
const saveReviewHandleError = (dispatch, error) => {
  console.log(error);
  dispatch({ type: SAVE, status: ERROR });
  if (error.response) {
    if (error.response.status >= 500) dispatch(internalServerError());
